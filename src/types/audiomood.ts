...
export type AnimationId='ripple'|'constellation'|'ribbon';
...
export interface AudioMoodSample{id:string;markerId:string;sourceStart:number;sourceEnd:number;duration:number;type:SampleKind;mood?:MoodId;animation?:AnimationId;audioFeatures:AudioFeatures;visualSeed:number;loop?:{start:number;end:number;crossfade:number}}
...
export const ANIMATIONS:{id:AnimationId;label:string;glyph:string}[]=[{id:'ripple',label:'Ondes',glyph:'◉'},{id:'constellation',label:'Étoiles',glyph:'✦'},{id:'ribbon',label:'Rubans',glyph:'〰'}];
export const moodById=(id?:MoodId)=>MOODS.find(m=>m.id===id)??MOODS[0];
